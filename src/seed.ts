...
export const seedTasks: SprintTask[] = [
  ...
  t('w1-wed-both', 1, W1[2], 'Wed', '14:30', '16:00', 'both',
    ...
    'Combine workflow map + pain points. Score each opportunity on feasibility x hours saved x revenue protected. Surface the top 5 to 7 candidates for the pilot Implementation. Distinguish (a) workflows we automate, (b) tools we reconfigure or replace, (c) work the founder should hand off elsewhere. Feeds Thursday Audit Framework. Forbid round-number hours-saved claims, use ranges (§11 Tier B).'),
  ...
  t('w1-fri-a', 1, W1[4], 'Fri', '13:00', '14:30', 'a',
    ...
    'Lock v0.4 pricing in writing for site copy and SOWs: Audit $750 (credited toward Implementation, classified as CAC not profit), Implementation Starter $4,500 / Standard $8,000 / Comprehensive $12,000, Managed Lite $750 / Standard $1,200 / Pro $2,000 per month. Payment terms: 50% on signature / 50% on UAT (Implementation); monthly auto-debit (Managed). Hour caps: Lite 1.5 hrs (overflow $150/hr), Standard 4 hrs, Pro 8 hrs. Capacity caps: max 1 active Implementation per founder per week; max 6 Lite-equivalents per founder for Managed. See v0.4 §3.'),
  t('w1-fri-b', 1, W1[4], 'Fri', '13:00', '14:30', 'b',
    'Implementation methodology document',
    'Document how we deliver an Implementation. 5-week template: Week 1 design + access provisioning, Weeks 2 to 3 build in Make / Zapier / n8n / Claude Code with §11 QC gates, Week 4 client UAT, Week 5 handoff with training video. Hard rules: every Implementation must include at least one capability Make AI Agents cannot natively deliver (forced rebuild check before pricing, if rebuild takes < 8 hrs, drop scope or drop deal); no new client monthly costs over $50/mo without written approval. See v0.4 §3.2.'),
  t('w1-fri-both', 1, W1[4], 'Fri', '14:30', '16:00', 'both',
    'Service Architecture Document (single source of truth)',
    'Merge pricing (§3) + methodology + delivery architecture (§7, who does what between Claude and humans). Locks end-of-Week-1 so Week 2 contract drafting has a fixed target. Save in deliverables/. This is what Grant and you reference when a client asks what we actually do.'),

  // --- Week 2: Business Infrastructure ---
  t('w2-mon-a', 2, W2[0], 'Mon', '13:00', '14:30', 'a',
    'MSA and SOW templates (v1)',
    'MSA (Master Services Agreement) = umbrella contract governing the whole client relationship. SOW (Statement of Work) = per-engagement scope. SOW must include: fixed price, scope deliverables, payment terms (50% on signature / 50% on UAT), per-engagement liability cap = fees paid (§11), and the v0.4 AI disclosure language: "Deliverables are produced using AI tooling and reviewed by Dream Helpers for source-traceability and internal consistency. Clients are responsible for validating numerical claims against their own systems before acting on recommendations." Get a tech E&O insurance quote in parallel (about $400 to $800/yr).'),
  t('w2-mon-b', 2, W2[0], 'Mon', '13:00', '14:30', 'b',
    'Client onboarding process',
    'From signed SOW to kickoff. Checklist of access provisioning (auth to client tools), data transfer protocol (where exports land), kickoff comms (welcome email, scheduling), shared workspace setup (Notion or Drive), kickoff call agenda. Target: under 24 hrs from signed SOW to we-have-what-we-need-to-start.'),
  t('w2-mon-both', 2, W2[0], 'Mon', '14:30', '16:00', 'both',
    'Review and finalize business documents',
    'Cross-review MSA, SOW, onboarding checklist. Lock language. Save final templates in deliverables/templates/. These ship as v1, we will re-edit from real-world experience.'),
  ...
  t('w2-thu-a', 2, W2[3], 'Thu', '13:00', '14:30', 'a',
    ...
    'Pages: Home (lead with vet positioning, secondary insurance, tertiary Shopify), Services, Audit ($750 details, credit-back mechanic, 5-day cadence), Case Studies (placeholder + Soil Detective once shippable), Contact, Who-we-will-not-help (§8, explicit). Publish pricing on the homepage (§13 differentiator, no competitor we found does this). Lead with §2 positioning statement: "We give independent veterinary practice managers back 10 to 20 hours per month by installing AI-powered operating systems on top of their existing PIMS and tools, without making them switch software or learn anything new."'),
  t('w2-thu-b', 2, W2[3], 'Thu', '13:00', '14:30', 'b',
    'Website design and build',
    'Astro + Tailwind shell, matching existing dreamhelpers.org pattern. Sections: hero with positioning statement, three-tier transparent pricing table, what-we-do outcomes (the four §4 metrics), who we serve, who we will not help (§8), sample audit page, Calendly-embed discovery booking, contact form. Mobile-first. Hosted same as dreamhelpers.org.'),
  t('w2-thu-both', 2, W2[3], 'Thu', '14:30', '16:00', 'both',
    'Review and launch website draft',
    'Both founders walk through site as a prospect would. §11 Tier A smell test on all copy. Test discovery booking flow end-to-end. Capture broken or awkward items for Friday cleanup. Soft-launch on production URL.'),

  t('w2-fri-both', 2, W2[4], 'Fri', '13:00', '16:00', 'both',
    'Website forms + scheduling + CRM integration',
    'Wire contact form -> CRM (webhook or native integration). Wire Calendly -> CRM with auto-tag of source. Build Stripe payment link page for the $750 audit. Add Plausible or Fathom analytics. Final QA pass. Push to public, site is live.'),

  // --- Week 3: Soil Detective Pilot Proof of Concept ---
  t('w3-mon-a', 3, W3[0], 'Mon', '13:00', '14:30', 'a',
    'Analyze Soil Detective customer service data',
    'Pull 30 to 90 days of email and inquiry data. Categorize by intent (question, complaint, sales, billing, support, partnership outreach, spam). Measure: volume per day, response time, resolution time, top categories, percent auto-deflectable. This is the baseline for the case study before metrics. §11 Tier B applies to any number that ends up in the audit or case study.'),
  t('w3-mon-b', 3, W3[0], 'Mon', '13:00', '14:30', 'b',
    'Analyze Soil Detective business and systems data',
    'Site analytics, sales / order data, marketing reach, current systems and tool stack. Map where the founder time goes weekly. Identify the 2 to 3 highest-leverage workflows worth featuring in the public case study (per §10 decision 12, do not publish we-built-the-whole-business framing).'),
  t('w3-mon-both', 3, W3[0], 'Mon', '14:30', '16:00', 'both',
    'Automation Opportunity Report (Soil Detective)',
    'Top 5 to 7 opportunities ranked by hours saved x revenue protected x feasibility. Each item: current process, proposed process, tools required, time-to-build, time-to-payback estimate, ROI range. This is the audit deliverable for the pilot, and the template we will reuse for every paid audit.'),

  t('w3-tue-a', 3, W3[1], 'Tue', '13:00', '14:30', 'a',
    'Build support category taxonomy (Soil Detective)',
    'Labels the AI classifier will route on. Include: question types (soil testing, product, shipping, account, technical), complaint types, sales inquiry, billing, support, partnership outreach, spam. Each label gets 3 to 5 example messages from real Soil Detective data. Drives Wednesday prototype.'),
  t('w3-tue-b', 3, W3[1], 'Tue', '13:00', '14:30', 'b',
    'Build response workflow map (Soil Detective)',
    'For each category in the taxonomy: who / what / when. Some categories get AI-drafted-human-reviewed replies; some get templated auto-replies; some escalate immediately. Document the routing logic explicitly, this is the spec the prototype implements.'),
  t('w3-tue-both', 3, W3[1], 'Tue', '14:30', '16:00', 'both',
    'Design support automation architecture',
    'End-to-end design: inputs (email + contact form + chat) -> classifier -> routing -> response generator -> escalation. Pick host environment (Make AI Agents if feasible per §3.2 rebuild-check; Claude Code if not). Decide where logs live so we can capture Week 3 metrics for the case study.'),

  t('w3-wed-both', 3, W3[2], 'Wed', '13:00', '16:00', 'both',
    'Build first customer service prototype',
    'Working version against real Soil Detective data. Should auto-classify with category labels from Tuesday, draft replies for around 50%+ of inbound, and route the remainder to human review. Test on backlog first, then a 24-hr supervised live-trial. Apply §11 QC to any number we report from this build.'),

  t('w3-thu-a', 3, W3[3], 'Thu', '13:00', '14:30', 'a',
    'Test email classification workflows',
    'Accuracy spot-check on 50 to 100 real messages. Target: at least 85% category accuracy. Mark every misclassification, identify pattern, iterate on the prompt or taxonomy. Document accuracy number with the methodology, this number will appear in the case study and the v0.4 §11 Tier B re-derivation requirement applies.'),
  t('w3-thu-b', 3, W3[3], 'Thu', '13:00', '14:30', 'b',
    'Test response generation workflows',
    'Quality + tone review of AI-drafted replies. Specifically check: did it commit to anything we should not have (refund amounts, delivery dates, technical claims)? Did it cite a number that is wrong? Does the tone match Soil Detective existing voice? Sample 20 to 30 drafts across categories. Flag anything that would have been a problem if sent unsupervised.'),
  t('w3-thu-both', 3, W3[3], 'Thu', '14:30', '16:00', 'both',
    'Review and refine prototype',
    'Process what testing surfaced. Adjust prompts, taxonomy, routing rules. Update the health report template for Managed Services (this is where the monthly metrics will come from after handoff).'),

  t('w3-fri-both', 3, W3[4], 'Fri', '13:00', '16:00', 'both',
    'Soil Detective case study + demo environment',
    'Case study: before / after metrics (response time, classification accuracy, hours reclaimed estimate with range), narrative, screenshots, founder quote if obtainable. Public version highlights 2 to 3 highest-leverage workflows ONLY, not the full build (per §10 decision 12). Demo environment: prospect-facing sandbox we can walk through in 5 min on a discovery call. Both go into Week 4 sales assets.'),
  ...
  t('w4-tue-a', 4, W4[1], 'Tue', '13:00', '14:30', 'a',
    ...
    '10-slide vet-version deck (lead with PE/EBITDA-to-exit math, named pains: staff churn, refund backlog, after-hours triage); parallel insurance-version (aggregator / EBITDA math, AMS-specific pains: renewal chase, x-date follow-up); parallel Shopify-version (post-Sidekick orchestration, what cross-system work CFSB and Sidekick cannot touch). Each deck: problem -> outcome -> process -> pricing -> next step.'),
  t('w4-tue-b', 4, W4[1], 'Tue', '13:00', '14:30', 'b',
    'Demo environment',
    'Sandboxed, shareable demo that runs the Soil Detective prototype concepts but with placeholder data we control. Screenshot-ready and walkthrough-able in 5 min during a discovery call.'),
...
export const seedMilestones: Milestone[] = [
  m('ms-w1', 1, 'Week 1 Foundation closed',
    'All five Week 1 deliverables shipped: Market Landscape Report, Pain Point Map (Soil Detective), Audit Framework (repeatable 5-day process), Service Packages priced + locked, Implementation methodology documented. Service Architecture Document is the merged single-source-of-truth artifact. Lock by EOD Fri so Week 2 contract drafting has a fixed target.',
    W1[4]),
  m('ms-w2', 2, 'Week 2 Infrastructure closed',
    'Website live with public pricing (§13 differentiator), MSA + SOW templates signed-ready with §11 AI disclosure language and per-engagement liability cap, CRM operational with all three segment pipelines, SOPs documented in deliverables/sops/, onboarding checklist ready. Tech E&O insurance quote secured. Stripe payment link page for $750 audit live.',
    W2[4]),
  m('ms-w2-data', 2, 'Soil Detective data access secured',
    'Critical gating dependency. By EOD Fri Week 2 we need 30 to 90 days of email and inquiry data, business and systems data, and tool access in a shared folder. If not received: Week 3 stalls and we burn the sprint. Mitigate by asking for data on Mon Week 2 and chasing daily.',
    W2[4]),
  m('ms-w3', 3, 'Week 3 POC closed',
    'Soil Detective prototype running: > 85% classification accuracy on real data, draft replies on around 50%+ of inbound (supervised), live-trial passed, case study draft written with 2 to 3 highest-leverage workflows highlighted (NOT the whole build), demo environment screenshot-ready for Week 4 sales calls.',
    W3[4]),
  m('ms-w4', 4, 'Week 4 GTM closed',
    'Prospect database in CRM (vet primary, insurance + Shopify parallel), pitch deck / demo / one-pager finished and QC-passed, 3+ discovery calls booked from cold outbound + Loom teardowns, conversion metrics being tracked against v0.4 §5 falsification triggers. Day-45 and Day-90 indicators (insurance swap test, Shopify kill criterion) on the bi-weekly review schedule.',
    W4[4]),
];
